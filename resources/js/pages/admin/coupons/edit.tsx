...
const Edit: React.FC<EditCouponProps> = ({ coupon, errors }) => {
  const [usageLimit, setUsageLimit] = useState<boolean>(!!coupon.usage_limit);
  const [dateRange, setDateRange] = useState<boolean>(!!(coupon.starts_at || coupon.expires_at));
  ...
  const handleSubmit: FormEventHandler = (e) => {
    ...
    post(route('admin.coupons.update', coupon.id), formData);
  };

  return (
    <AdminLayout title={`Edit Coupon: ${coupon.code}`}>
        ...
        <div className="md:grid md:grid-cols-3 md:gap-6">
          <div className="md:col-span-1">
            <div className="px-4 sm:px-0">
              <h3 className="text-lg font-medium leading-6 text-gray-900">Edit Coupon</h3>
              <p className="mt-1 text-sm text-gray-600">
                Update the details of this discount coupon. Changes will be available immediately to customers.
              </p>
              <div className="mt-6">
                ...
                <div className="mt-6 border-t border-gray-200 pt-4">
                  <h4 className="text-sm font-medium text-gray-900">Coupon Usage</h4>
                  <p className="mt-1 text-sm text-gray-600">
                    This coupon has been used <span className="font-medium">{coupon.used_count}</span> time{coupon.used_count !== 1 ? 's' : ''}.
                  </p>
                </div>
              </div>
            </div>
